...
public class GameController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _BgController = FindObjectOfType<BackgoundSoundController>();

        // Bắt đầu phát âm thanh nền
        _BgController.PlayBackground(true);
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (_GameState._IsPlayer && !_GameState._IsPlayerStart)
            {
                ...
                _cameraController.MoveCameraToBullet(_GameObj.transform.position);
                _mapController.MoveMapToBullet(_GameObj.transform.position);
                _cameraController._IsMove = true;
                _mapController._IsMove = true;
            }
        }
    ...
    public void MoveCameraToObj()
    {
        ...
        _cameraController.MoveCameraToBullet(_GameObj.transform.position);
        _mapController.MoveMapToBullet(_GameObj.transform.position);
        _cameraController._IsMove = false;
        _mapController._IsMove = false;
    }
    /// <summary>
