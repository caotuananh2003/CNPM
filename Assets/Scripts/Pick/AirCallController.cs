...
public class AirCallController : MonoBehaviour
{
    ...
    private void Pick_Bomb()
    {
        ...
        { // Khi nhấn chuột trái
            ...
            if (_GameController._GameObj.tag == "Player")
                _GameController._ListStatePickPlayer[3].Ammo -= 1;
            else
                _GameController._ListStatePickEnemy[3].Ammo -= 1;
    ...
    public void Pick_BombEnemy(Vector3 target)
    {
        ...
        _GameController._CheckHit = true; // Đánh dấu đã thả bom
    ...
    private void Pick_Teleport()
    {
        ...
        { // Khi nhấn chuột trái.
            ...
            if (_GameController._GameObj.tag == "Player")
                _GameController._ListStatePickPlayer[9].Ammo -= 1;
            else
                _GameController._ListStatePickEnemy[9].Ammo -= 1;

            // Phát âm thanh teleport và khởi tạo hiệu ứng dịch chuyển.
            _SoundController.PlayTeleportSound(true);
    ...
    IEnumerator waitPick_Teleport_Off()
    {
        ...
        _SoundController.PlayTeleportSound(true); // Phát âm thanh teleport lần nữa.
    ...
    IEnumerator waitPick_Teleport_On()
    {
        ...
        _GameController._GameObj.transform.position = _posDes; // Dịch chuyển đối tượng tới vị trí đích đã được xác định.
        ...
        _GameController._CheckHit = true; // Đánh dấu rằng người chơi đã thực hiện hành động thành công.
        _UIManager.ResetImgPick(); // Đặt lại giao diện chọn vũ khí.
        _GameController.ChangeTurn(); // Chuyển lượt cho đối thủ.
    ...
    public void ActivePick_Swap()
    {
        if (Input.GetMouseButtonDown(0)) // Kiểm tra nếu người chơi nhấn chuột trái.
        {
            ...
            if (obj != null && obj.tag == "Swap")
            {
                ...
                if (_GameController._GameObj.tag == "Player")
                    _GameController._ListStatePickPlayer[15].Ammo -= 1;
                else
                    _GameController._ListStatePickEnemy[15].Ammo -= 1;

                _GameController._GameObj = obj.transform.parent.gameObject; // Thiết lập đối tượng GameObject để hoán đổi.

                // Kích hoạt lượt cho đối tượng vừa hoán đổi.
                _GameController.SetMyTurnForObj(_GameController._GameObj);
    ...
    public void Pick_Health()
    {
        ...
        if (_GameController._GameObj.tag == "Player")
            _GameController._ListStatePickPlayer[14].Ammo -= 1;
        else
            _GameController._ListStatePickEnemy[14].Ammo -= 1;

        // Tạo đối tượng Health tại vị trí của người chơi hoặc đối thủ để hồi máu.
        GameObject health = (GameObject)Instantiate(Pick[4], _GameController._GameObj.transform.position, Quaternion.identity);
        health.transform.parent = _GameController._GameObj.transform; // Gán đối tượng hồi máu vào người chơi hoặc đối thủ.

        // Nếu đối tượng đang bị trúng độc, hủy bỏ hiệu ứng trúng độc.
        Glow_PoisonController poison = _GameController._GameObj.GetComponentInChildren<Glow_PoisonController>();
        if (poison != null) Destroy(poison.gameObject); // Hủy bỏ đối tượng điều khiển hiệu ứng độc.

        Player player = _GameController._GameObj.GetComponent<Player>(); // Lấy thông tin Player.
