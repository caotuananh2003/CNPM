...
public class Pick_GrenadeControler : MonoBehaviour
{
    ...
    GameControler _GameControler;  // Truy cập GameControler
    ...
    SoundControler _SoundControler;  // Truy cập SoundControler

    //---------------------------------------	
    void Start()
    {
        trajectoryPoints = new List<GameObject>();  // Khởi tạo danh sách điểm quỹ đạo
        _GameControler = FindObjectOfType<GameControler>();  // Tìm GameControler trong game
        _UIManager = FindObjectOfType<UIManager>();  // Tìm UIManager trong game
        _SoundControler = FindObjectOfType<SoundControler>();  // Tìm SoundControler trong game
        ...
        if (isBallThrown && numText != null)
        {
            ...
            if (_timeCount > 1)
            {
                ...
                if (_timeDestroy == 0)
                {
                    ...
                    _SoundControler.PlayExplosionSound(true);  // Phát âm thanh nổ
    ...
    private void throwBall()
    {
        ...
        _GameControler._CheckHit = true;  // Cập nhật trạng thái trúng đích
        _GameControler._StopTime = true;  // Dừng thời gian
